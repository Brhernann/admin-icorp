...
import {columns5} from '../../../JSON';
import ReactHtmlParser from 'react-html-parser';

class f2Persona extends Component {
  constructor(props) {
    super(props);
  ...
  CreateTableEv(_array, _array1) {
    let obj = _array.map((e, i) => {
      ...
      console.log('tabla de f2 persona', ob)
      // _array1.forEach(
      //   (element, index) =>
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Divider orientation="left">Encuesta de flujo de fase 2 de encuesta de persona</Divider>

        <div style={{ background: 'white', padding: '10px' }}>
          <Table loading={loading} columns={columns5} dataSource={dataTable} scroll={{ x: 300, y: 300 }} />
        </div>
      </div>
    );
  }
...
const mapDispatchToPropsAction = dispatch => ({
  ...
  FetchView: value => dispatch(_GetF2PersonaView(value)),
